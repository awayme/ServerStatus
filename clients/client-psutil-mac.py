...
def get_hdd():
    # valid_fs = [ "ext4", "ext3", "ext2", "reiserfs", "jfs", "btrfs", "fuseblk", "zfs", "simfs", "ntfs", "fat32", "exfat", "xfs", "apfs" ]
    # disks = dict()
    # size = 0
    # used = 0
    # for disk in psutil.disk_partitions():
    #     # print(disk.device)
    #     # print(disk.fstype.lower())
...
def tupd():
    ...
    if 'linux' in sys.platform:
        ...
        t = int(os.popen('netstat -an|grep "tcp" | wc -l').read()[:-1])-1
        u = int(os.popen('netstat -an|grep "udp" | wc -l').read()[:-1])-1
        p = len(psutil.pids())
        d = 0
